...
def generar_recibo_santa_paula(residente, inmueble, fecha, referencia, monto):
    # Diseño HTML y CSS adaptado estrictamente para xhtml2pdf
    html_recibo = f"""
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .header-table {{
                width: 100%;
                border-bottom: 2px solid #1C3F60; /* Azul Marino Elegante */
                padding-bottom: 15px;
                margin-bottom: 30px;
            }}
            .title {{
    ...
    # El nombre del archivo ahora incluye el inmueble para tener mejor orden
    nombre_archivo = f"Recibo_SantaPaula_Apto{inmueble}_Ref{referencia}.pdf"
    
    with open(nombre_archivo, "w+b") as archivo_pdf:
        pisa_status = pisa.CreatePDF(html_recibo, dest=archivo_pdf)
    
    if pisa_status.err:
        print("❌ Hubo un error al generar el PDF del condominio.")
    else:
        print(f"✅ ¡Recibo creado y guardado como: {nombre_archivo}!")
